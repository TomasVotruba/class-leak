 ...
 */
use ClassLeak202410\PhpParser\Node\Arg;
use ClassLeak202410\PhpParser\Node\Expr;
use ClassLeak202410\PhpParser\Node\Expr\Array_;
use ClassLeak202410\PhpParser\Node\Expr\Cast\Double;
use ClassLeak202410\PhpParser\Node\Identifier;
use ClassLeak202410\PhpParser\Node\InterpolatedStringPart;
use ClassLeak202410\PhpParser\Node\Name;
use ClassLeak202410\PhpParser\Node\Param;
use ClassLeak202410\PhpParser\Node\PropertyHook;
use ClassLeak202410\PhpParser\Node\Scalar\InterpolatedString;
use ClassLeak202410\PhpParser\Node\Scalar\Int_;
use ClassLeak202410\PhpParser\Node\Scalar\String_;
use ClassLeak202410\PhpParser\Node\Stmt;
use ClassLeak202410\PhpParser\Node\Stmt\Class_;
use ClassLeak202410\PhpParser\Node\Stmt\ClassConst;
use ClassLeak202410\PhpParser\Node\Stmt\ClassMethod;
use ClassLeak202410\PhpParser\Node\Stmt\Else_;
use ClassLeak202410\PhpParser\Node\Stmt\ElseIf_;
use ClassLeak202410\PhpParser\Node\Stmt\Enum_;
use ClassLeak202410\PhpParser\Node\Stmt\Interface_;
use ClassLeak202410\PhpParser\Node\Stmt\Namespace_;
use ClassLeak202410\PhpParser\Node\Stmt\Nop;
use ClassLeak202410\PhpParser\Node\Stmt\Property;
use ClassLeak202410\PhpParser\Node\Stmt\TryCatch;
use ClassLeak202410\PhpParser\Node\UseItem;
use ClassLeak202410\PhpParser\NodeVisitor\CommentAnnotatingVisitor;
abstract class ParserAbstract implements Parser
{
    ...
    */
    ...
    protected function createCommentFromToken(Token $token, int $tokenPos) : Comment
    {
        ...
        return \T_DOC_COMMENT === (\is_array($token) ? $token[0] : $token) ? new Comment\Doc(\is_array($token) ? $token[1] : $token, $token->line, $token->pos, $tokenPos, $token->getEndLine(), $token->getEndPos() - 1, $tokenPos) : new Comment(\is_array($token) ? $token[1] : $token, $token->line, $token->pos, $tokenPos, $token->getEndLine(), $token->getEndPos() - 1, $tokenPos);
    }
    /**
