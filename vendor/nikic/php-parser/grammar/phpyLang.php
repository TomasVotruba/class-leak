...
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \ClassLeak202307\PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = resolveNodes($matches['params']);
        $params = magicSplit('(?:' . \ClassLeak202307\PARAMS . '|' . \ClassLeak202307\ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ClassLeak202307\ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = magicSplit('(?:' . \ClassLeak202307\PARAMS . '|' . \ClassLeak202307\ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
...
{
    return '~' . \ClassLeak202307\LIB . '(?:' . \str_replace('~', '\\~', $regex) . ')~';
}
