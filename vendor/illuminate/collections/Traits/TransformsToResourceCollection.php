...
namespace ClassLeak202605\Illuminate\Support\Traits;

use ClassLeak202605\Illuminate\Database\Eloquent\Attributes\UseResource;
use ClassLeak202605\Illuminate\Database\Eloquent\Attributes\UseResourceCollection;
use ClassLeak202605\Illuminate\Database\Eloquent\Model;
use ClassLeak202605\Illuminate\Http\Resources\Json\ResourceCollection;
use LogicException;
use ReflectionClass;
trait TransformsToResourceCollection
{
    ...
    protected function resolveResourceFromAttribute(string $class) : ?string
    {
        ...
        $attributes = \method_exists(new ReflectionClass($class), 'getAttributes') ? (new ReflectionClass($class))->getAttributes(UseResource::class) : [];
    ...
    protected function resolveResourceCollectionFromAttribute(string $class) : ?string
    {
        ...
        $attributes = \method_exists(new ReflectionClass($class), 'getAttributes') ? (new ReflectionClass($class))->getAttributes(UseResourceCollection::class) : [];
        return $attributes !== [] ? $attributes[0]->newInstance()->class : null;
    }
}
