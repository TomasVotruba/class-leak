...
use ArrayAccess;
use ArrayIterator;
use ClassLeak202605\Illuminate\Contracts\Support\CanBeEscapedWhenCastToString;
use ClassLeak202605\Illuminate\Support\Traits\EnumeratesValues;
use ClassLeak202605\Illuminate\Support\Traits\Macroable;
use ClassLeak202605\Illuminate\Support\Traits\TransformsToResourceCollection;
...
class Collection implements ArrayAccess, CanBeEscapedWhenCastToString, Enumerable
{
    ...
    {
        $values = (isset($key) ? $this->pluck($key) : $this)->reject(function ($item) {
            return \is_null($item);
        })->sort()->values();
        $count = $values->count();
        if ($count === 0) {
    ...
    public function mode($key = null)
    {
        ...
        $collection->each(function ($value) use($counts) {
            return $counts[$value] = isset($counts[$value]) ? $counts[$value] + 1 : 1;
        });
        $sorted = $counts->sort();
    ...
    {
        return new static(Arr::crossJoin($this->items, ...\array_map(\Closure::fromCallable([$this, 'getArrayableItems']), $lists)));
    }
    /**
            ...
            foreach ($groupKeys as $groupKey) {
                switch (\true) {
                    case \is_bool($groupKey):
                        ...
                        break;
                    case $groupKey instanceof \UnitEnum:
                        $groupKey = enum_value($groupKey);
                        break;
                    case \is_object($groupKey) && \method_exists($groupKey, '__toString'):
                    case \is_null($groupKey):
