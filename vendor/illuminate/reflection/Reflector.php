...
class Reflector
{
    ...
    public static function getClassAttributes($objectOrClass, $attribute, $includeParents = \false)
    {
        ...
        $attributes = [];
        do {
            $attributes[$reflectionClass->name] = new Collection(\array_map(function (ReflectionAttribute $reflectionAttribute) {
                return $reflectionAttribute->newInstance();
            }, \method_exists($reflectionClass, 'getAttributes') ? $reflectionClass->getAttributes($attribute) : []));
        } while ($includeParents && \false !== ($reflectionClass = $reflectionClass->getParentClass()));
        \reset($attributes);
        return $includeParents ? new Collection($attributes) : $attributes[\key($attributes)];
    }
    /**
    ...
    public static function isParameterBackedEnumWithStringBackingType($parameter)
    {
        ...
        $backedEnumClass = ($nullsafeVariable1 = $parameter->getType()) ? $nullsafeVariable1->getName() : null;
        if (\is_null($backedEnumClass)) {
            return \false;
        }
        if (\class_exists($backedEnumClass)) {
