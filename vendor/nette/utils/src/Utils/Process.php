 ...
 */
namespace ClassLeak202605\Nette\Utils;

use ClassLeak202605\Nette;
/**
...
final class Process
{
    ...
    private function __construct($command, ?array $env, array $options, ?string $directory, $stdin, $stdout, $stderr, ?float $timeout)
    {
        ...
        $process = @\proc_open(\is_array($command) ? \implode(' ', \array_map('escapeshellarg', $command)) : $command, $descriptors, $pipes, $directory, $env, $options);
    ...
    private function createOutputDescriptor(int $id, $output)
    {
        if (\is_resource($output)) {
            ...
            return Helpers::IsWindows ? \tmpfile() : ['pipe', 'w'];
        } else {
